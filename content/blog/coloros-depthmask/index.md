...
下面三张图是 ColorOS 16 上的实际界面截图，分别对应锁屏编辑页入口、v0.1.4 的 replace-only 蒙版面板和完整蒙版编辑器。
...
<figure class="depthmask-screenshot">
  <a href="mask-panel.jpg" data-lightbox="true">
    <img src="mask-panel.jpg" alt="ColorOSDepthMask v0.1.4 蒙版功能面板，包含自动生成、编辑、导入、反转和删除蒙版" loading="lazy" decoding="async">
  </a>
  <figcaption>v0.1.4 蒙版面板：保留自动生成、编辑、导入、反转和删除蒙版，不再提供替换 / 并集 / 交集切换。</figcaption>
</figure>

点击编辑页底部的 **蒙版** 后，会直接打开当前壁纸对应的功能面板。这里提供 **自动生成、编辑、导入、反转和删除蒙版**；所有操作都绑定到当前壁纸，而不是一个全局蒙版。

这里已经没有“替换”按钮。原因是从 v0.1.4 开始，只要自定义蒙版存在，它天然就是最终前景定义，因此再让用户选择“替换”只会重复表达同一个状态。
...
因此蒙版面板现在只展示对当前自定义蒙版本身有意义的操作：**自动生成、编辑、导入、反转、删除蒙版**。不再显示“替换 / 并集 / 交集”分段控件。
